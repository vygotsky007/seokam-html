...
for (let i = 0; i < 14; i++) {
  ...
  SERIES.push({ key: 's' + (i + 1), forms: [CIRCLED_SYL[i], PAREN_SYL[i], SYL[i], '(' + SYL[i] + ')'] });
}
// A. B. C. / (A)(B) — canonToken 이 소문자로 낮추므로 소문자로 등록한다
for (let i = 0; i < ALPHA.length; i++) {
  SERIES.push({ key: 'a' + (i + 1), forms: [ALPHA[i], '(' + ALPHA[i] + ')'] });
}
// OX 문항: O/○ 와 X/× 를 각각 한 계열로
SERIES.push({ key: 'ox_o', forms: ['o', '○', '〇', '동그라미'] });
SERIES.push({ key: 'ox_x', forms: ['x', '×', '✕', '엑스'] });
...
// 답 하나 → 토큰 배열. 기본은 정렬(순서 무시: "ㄷ,ㄱ" = "ㄱ,ㄷ").
// 순서 배열형(type=order)은 순서 자체가 답이므로 ordered=true 로 정렬하지 않는다.
function tokenize(v, ordered) {
  if (v == null) return [];
  const toks = splitAnswers(toHalfWidth(String(v)).replace(/[→>]/g, ',')).map(canonToken).filter(Boolean);
  return ordered ? toks : toks.sort((a, b) => (a.kind + a.v).localeCompare(b.kind + b.v));
}
...
function matchesOne(given, expected, ordered) {
  const g = tokenize(given, ordered), e = tokenize(expected, ordered);
  if (!e.length || g.length !== e.length) return false;
  return e.every((t, i) => tokenEq(g[i], t));
}

// 정답(파이프로 여러 개 등록 가능)과 학생답 비교
// opts.ordered = true → 순서 배열형(㉢→㉠→㉡): 순서가 다르면 오답
function isCorrect(given, expected, opts) {
  ...
  return String(expected == null ? '' : expected)
    ...
    .map((x) => x.trim())
    .filter(Boolean)
    .some((alt) => matchesOne(given, alt, ordered));
}
...
// 맞은 토큰 개수(부분 채점용). 선긋기의 '맞은 쌍 수', 복수 답의 '맞은 개수' 를 데이터로 남긴다.
// ※ 부분 점수를 주지는 않는다(배점 기능이 없다) — 오답이되, 몇 개 맞았는지는 저장해 둔다.
function partialCount(given, expected) {
  const alts = String(expected == null ? '' : expected).split('|').map((x) => x.trim()).filter(Boolean);
  let best = 0;
  for (const alt of alts) {
    const e = tokenize(alt);
    const g = tokenize(given);
    const used = new Array(g.length).fill(false);
    let hit = 0;
    for (const t of e) {
      for (let i = 0; i < g.length; i++) {
        if (!used[i] && tokenEq(g[i], t)) { used[i] = true; hit++; break; }
      }
    }
...
const api = { normalize, tokenize, isCorrect, isNearMiss, editDistance, partialCount };
